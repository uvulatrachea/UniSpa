...
export default function StaffShell({ title, subtitle, children }) {
  ...
  const staffType = String(auth?.staff?.staff_type || "").toLowerCase();

  const modules = useMemo(
    () => [
      { label: "Dashboard", href: route("staff.dashboard"), icon: "fa-chart-line" },
      ...(staffType === "student"
        ? [{ label: "My Availability", href: route("staff.availability"), icon: "fa-calendar-days" }]
        : []),
    ],
    [staffType]
  );
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            <div>
              ...
              <h2 className="text-lg font-extrabold text-unispa-primaryDark">Staff Panel</h2>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold transition ${
                    isActive ? "bg-unispa-primaryDark text-white shadow" : "text-slate-700 hover:bg-slate-100"
                  }`}
